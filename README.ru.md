...
- Полноэкранный «Сейчас играет» (`f`): прогресс, битрейт, очередь, статус
- Режимы повтора (`r`): выкл / всё / один. Таймер сна (`n`): от 15 до 120 минут
- Лайк/анлайк треков (`l` / `L`) и альбомов (`l` в списке альбомов), дизлайк (`d` / `D`)
- Копирование ссылки на трек (`ctrl+s`) и на плейлист (`Y`)
- Синхронные и текстовые лирики (`t`), переключение качества (`q`): best / high / medium / low с битрейтом
- Кэширование треков (`c`), менеджер кэша (`B`), кэширование всех лайков (`C`), скачивание в файл (`ctrl+w`)
- Скачанное сохраняется в лучшем качестве, с обложкой и полными ID3-тегами
- Вкладки: Playlists, Radio (`R`), Search (`S`)
- Моя волна остаётся в общем списке; остальные станции — на вкладке Radio
- История прослушиваний (последние 100 треков, сохраняется между запусками), очередь (`tab`), переход к играющему (`N`)
- Просмотр исполнителя (`i`), переход к альбому (`A`), информация о треке (`I`)
- Добавление в плейлист (`a`), быстрое добавление в последний (`y`), удаление из очереди без анлайка (`X`), перемещение (`ctrl+u` / `ctrl+d`)
- Сортировка (`s`): название / исполнитель / длительность. Фильтр списка (`/`, `esc` очищает)
- Глобальный поиск (треки/исполнители/альбомы, `tab` — фильтр): `enter` играет/открывает, `p` — следующим
- Экспорт в M3U (`E`, локальные пути для кэша), импорт (`U`): MP3/M3U-файлы и папки
- Восстановление сессии: очередь, трек и позиция переживают перезапуск (`resume-on-start`)
- Цветовые темы (`purple`/`amber`/`mono`), тост статистики (`ctrl+g`), подтверждения (`y`/`n`)
- Управление запущенным экземпляром для скриптов: `wellya --toggle/--next/--prev/--play/--pause/--stop`
- Полная справка (`F1`), все клавиши переназначаются в конфиге
...
```yaml
style:
    theme: purple # purple/amber/mono; заменяет всю палитру
```

Без `theme` (или `custom`) используется свой блок `colors:`.
